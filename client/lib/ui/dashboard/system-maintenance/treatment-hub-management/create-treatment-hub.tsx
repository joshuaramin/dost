...
export default function CreateTreatmentHub() {
  ...
  const { data: RegionalData } = useFormQuery<RegionsInterfaceResult>({
    key: ["GetAllRegions"],
    url: "maintenance/geospatial/regions",
    headers,
  });

  const { data: ProvincesData } = useFormQuery<ProvinceInterfaceResult>({
  ...
  const { data: MunicipalitiesData } = useFormQuery<ProvinceInterfaceResult>({
  ...
  const { data: BarangayData } = useFormQuery<BarangayInterfaceResult>({
    ...
    mutation.mutate(data, {
      onSuccess: () => {
        activityMutation.mutate(
          {
            type: "CREATE",
            description: `User created a new treatment hub: ${data.name}.`,
      ...
      <Form onSubmit={handleSubmit(onHandleSubmit)}>
        <div className={styles.container}>
          ...
          <Select
            ...
            label="Region"
            name="region_ogc_fid"
            options={(RegionalData?.data.data || []).map(({ id, name }) => ({
              label: name,
              value: id,
            }))}
          ...
          {region && (
            <Select
              ...
              label="Province"
              name="province_ogc_fid"
              options={(ProvincesData?.data.data || []).map(({ id, name }) => ({
                label: name,
                value: id,
              }))}
          ...
          {province && (
            <Select
              ...
              label="Municipality"
              name="municipality_ogc_fid"
              options={(MunicipalitiesData?.data.data || []).map(
                ({ id, name }) => ({
                  label: name,
                  value: id,
                }),
          ...
          {municipality && (
            <Select
              ...
              options={(BarangayData?.data.data || []).map(({ id, name }) => ({
                label: name,
                value: id,
              }))}
          ...
          <Select
            ...
            options={(TreatmentHubServicesData?.data.edges || []).map(
              ({ node }) => ({
                label: node.name,
          ...
          <div className={styles.footer}>
            <Button types="filled" type="submit" size="md" variant="primary">
