...
export default function QuestionCard({
  ...
  const questionId = question?.survey_question_id ?? survey_question_id;
  ...
  const addMoreMutation = useFormMutation<
  ...
  const deleteQuestionMutation = useFormMutation({
    key: ["SoftDelete", questionId ?? `question-${fieldId}`],
    method: "PATCH",
    url: questionId ? `maintenance/survey/${survey_question_id}` : "",
    headers,
  });

  const updateQuestionMutation = useFormMutation({
    key: ["UpdateQuestion", questionId ?? `question-${fieldId}`],
    method: "PATCH",
    url: questionId ? `maintenance/survey/question/${survey_question_id}` : "",
    headers,
  });
  ...
  const handleUpdateCard = () => {
    ...
    const currentQuestionId =
      currentQuestion.survey_question_id ?? survey_question_id;

    if (!currentQuestionId) {
      return;
    }

    const currentType = currentQuestion.type ?? "SHORT_TEXT";
    ...
    const payload = {
      ...
      type: currentType,
      is_required: currentQuestion.is_required === true,
      order_index: index + 1,
  ...
  return (
    <div
      ...
      data-question-id={questionId ?? fieldId}
    >
        ...
        <div className={styles.body}>
          <div className={styles.questions}>
            ...
            {questionType === "LONG_TEXT" && (
              <TextareaReadOnly readOnly placeholder="Long Answer" />
            )}
      ...
      <div className={styles.footer}>
        <div className={styles.footer_1_col_1}>
          <Button
            types="outline"
            size="sm"
            variant={isOnlyQuestion ? "disabled" : "danger"}
            type="button"
            onClick={handleDeleteCard}
            disabled={isOnlyQuestion || deleteQuestionMutation.isPending}
          >
            <TbTrash size={20} />
          </Button>
          ...
          <Button
            ...
            disabled={!questionId || updateQuestionMutation.isPending}
          >
            <Text size="sm">
              {updateQuestionMutation.isPending ? "UPDATING..." : "UPDATE"}
            </Text>
          </Button>
        </div>
      </div>
      ...
      {isLast && (
        <div className={styles.add}>
          ...
          >
            <Text size="sm">
              {addMoreMutation.isPending ? "CREATING..." : "ADD MORE"}
            </Text>
          </Button>
        </div>
      )}
